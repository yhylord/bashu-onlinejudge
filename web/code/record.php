...
function is_in_contest($problem_id)
{
  $get_contests = "SELECT contest_id FROM contest_problem WHERE problem_id=$problem_id";
  while ($contest = mysql_fetch_row(mysql_query($get_contests))) {
    $get_time = "SELECT start_time, end_time FROM contest WHERE contest_id=$contest[0]";
    $time = mysql_fetch_assoc(mysql_query($get_time));
    $start_time = DateTime::createFromFormat('Y-m-d H:i:s', $time['start_time']);
    $end_time = DateTime::createFromFormat('Y-m-d H:i:s', $time['end_time']);
      ...
      <div class="row-fluid">
        <div class="span12">
          <form action="record.php" method="get" class="form-inline" id="form_filter">
            <label>Problem:</label>
            <input type="text" class="input-mini" name="problem_id" id="ipt_problem_id"
            ...
            <label class="checkbox"><input id="check-in-contest" type="checkbox" name="in_contest">In Contest</label>
          ...
          <table class="table table-hover table-bordered" style="margin-bottom:0">
            ...
            <tbody id="tab_record">
              ...
              while ($row = mysql_fetch_array($res)):
                if (($in_contest && !is_in_contest($row['problem_id'])) || (is_in_contest($row['problem_id']) && $_SESSION['user'] !== $row['user_id'])) {
                  continue;
                }
        ...
        <ul class="pager">
          <li>
            <a class="pager-pre-link shortcut-hint" title="Alt+A"
               href="record.php?<?php echo htmlspecialchars(get_pre_link()) ?>" id="btn-pre"><i
                class="icon-angle-left"></i> Previous</a>
          </li>
          <li>
            <a class="pager-next-link shortcut-hint" title="Alt+D"
               href="record.php?<?php echo htmlspecialchars(get_next_link()) ?>" id="btn-next">Next <i
    ...
    <script type="text/javascript">
      $(document).ready(function () {
          ...
          if (h.substr(0, 9) == '#sw_open_') {
            $.ajax({
              ...
              success: function (msg) {
                if (/success/.test(msg))toggle_s($target.find('i'));
              }
            });
            return false;
          } else if (h == '#uid') {
        ...
        $('#btn_reset').click(function () {
          window.location = "record.php?problem_id=" + $("#ipt_problem_id").val() + "&user_id=" + $("#ipt_user_id").val();
        });
      });
    </script>
  </body>
